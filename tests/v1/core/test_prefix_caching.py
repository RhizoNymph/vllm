...
def test_capture_clamps_prefix_cache_hit():
    ...
    computed, n0, _ = manager.get_computed_blocks(req0)
    assert n0 == 0
    ...
    _, n_clamped, _ = manager.get_computed_blocks(_capture_request("clamp", True, 20))
    assert n_clamped == block_size

    # Generated-only capture -> no clamp -> full 3-block hit.
    _, n_full, _ = manager.get_computed_blocks(
        _capture_request("generated", False, None)
    )
    assert n_full == 3 * block_size

    # Floor at 0 (e.g. all_prompt) -> no reusable prefix at all.
    _, n_zero, _ = manager.get_computed_blocks(_capture_request("all", True, 0))
    assert n_zero == 0

    # Unclassified (offline path) -> skip reuse entirely via the skip flag.
    _, n_offline, _ = manager.get_computed_blocks(
        _capture_request("offline", None, None)
    )
    assert n_offline == 0
...
def test_capture_serves_from_store_instead_of_reforwarding():
    ...
    try:
        ...
        _, n_served, _ = manager.get_computed_blocks(req1)
        assert n_served > 0
        ...
        for k in keys[:-1]:
            store.put(k, torch.zeros(2, dtype=torch.float32))
        req3 = _capture_req("3")
        _, n_clamped, _ = manager.get_computed_blocks(req3)
        assert n_clamped == 0
