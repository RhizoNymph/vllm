...
class TestGetRow:
    ...
    def test_unregistered_nonzero_prefill_raises(self):
        """An unknown nonzero hash with is_prefill=True raises RuntimeError."""
        mgr = _make_manager()
        with pytest.raises(RuntimeError, match="not registered"):
            mgr.get_row_for_config(config_hash=12345, is_prefill=True)

    def test_unregistered_nonzero_decode_raises(self):
        """An unknown nonzero hash with is_prefill=False raises RuntimeError."""
        mgr = _make_manager()
        with pytest.raises(RuntimeError, match="not registered"):
    ...
    def test_registered_hash_uses_phase_for_lookup(self):
        ...
        row = mgr.register_config(config_hash=42, vectors=vectors, phase="prefill")
        assert mgr.get_row_for_config(config_hash=42, is_prefill=True) == row
        # With is_prefill=False the (42, "decode") key is absent, raises
        with pytest.raises(RuntimeError, match="not registered"):
...
class TestDeferredDecodeRegistration:
    ...
    These tests validate the SteeringManager's capacity enforcement.
    """

    def test_decode_registration_raises_on_capacity(self):
...
class TestLazyInitCapacityExhaustion:
    ...
    allows.  The scheduler must guarantee capacity so this should
    never happen at runtime, but the manager must raise
    RuntimeError for defense in depth.
    """

    def test_prefill_overflow_raises(self):
        """Registering more prefill configs than capacity should raise."""
        max_cfgs = 2
        ...
        # The next registration should raise RuntimeError.
    ...
    def test_decode_overflow_raises(self):
        """Registering more decode configs than capacity should raise."""
        max_cfgs = 2
        mgr = _make_manager(max_configs=max_cfgs)
    ...
    def test_mixed_phase_overflow_graceful(self):
        ...
        succeed; subsequent ones raise RuntimeError.  The manager
    ...
    def test_overflow_raises_for_unregistered(self):
        """When a config cannot be registered due to capacity, the
        ``get_row_for_config`` call should raise RuntimeError.
        """
        max_cfgs = 1
        mgr = _make_manager(max_configs=max_cfgs)
