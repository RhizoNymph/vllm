...
dyn row; without cleanup the ``("dyn", dyn_id)`` monitor/scale entries would
...
def test_release_leaves_other_owners_intact():
    ...
    mgr.set_row_monitor(HOOK, 0, ("dyn", d1), torch.ones(HIDDEN), 0.0, 1.0)
    mgr.set_row_monitor(HOOK, 0, ("dyn", d2), torch.ones(HIDDEN), 0.0, 1.0)
    mgr.set_row_monitor(HOOK, 0, ("config", 7, "decode"), torch.ones(HIDDEN), 0.0, 1.0)
    ...
    assert ("dyn", d1) not in layers
    assert ("dyn", d2) in layers
    assert ("config", 7, "decode") in layers
