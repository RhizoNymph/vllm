...
class CohereDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # o_proj/down_proj run with reduce_results=False, so both branches are
        # partial sums under TP. The POST_ATTN intervention site observes the
        # post-attention residual, which is only well defined once the
        # attention branch is reduced, so the two branches are reduced
        # separately here rather than as a single fused all-reduce.
        if self.tp_size > 1:
            hidden_states_attention = tensor_model_parallel_all_reduce(
                hidden_states_attention
            )
            hidden_states_mlp = tensor_model_parallel_all_reduce(hidden_states_mlp)
