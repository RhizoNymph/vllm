...
class SteeringManager:
    ...
    def update_global_vectors(
        ...
        *,
        ...
        # because they are both produced and consumed on the step thread.
        # Synchronize here so the tensor's memory is fully materialized before
        # the manager exposes it for cross-thread reads. This is a rare
        # control-plane op, so the sync cost is irrelevant; the single-rank /
        # non-CUDA paths are unaffected.
        if stored.is_cuda:
