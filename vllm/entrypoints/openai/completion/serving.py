...
class OpenAIServingCompletion(OpenAIServing):
    ...
    async def create_completion(
        ...
        for i, engine_input in enumerate(engine_inputs):
            ...
            #
            # See ``OpenAIServingChat._admit_capture`` for the full
            # rationale of the ordering. Legacy completions admits
            # per-prompt: when a single request expands into multiple
            # engine inputs, each one gets its own admission check
    ...
    async def completion_stream_generator(
        ...
        try:
            ...
            if include_usage or capture_results is not None:
                ...
                # and/or the capture-consumer results. If only
                # ``capture_results`` is set (client did not request
                # usage), ``usage`` is left ``None`` so strict clients
                # see the same usage-absent envelope they always saw.
    ...
    def request_output_to_completion_response(
        ...
        # Capture-consumer results: legacy completions collapses the
        # multi-prompt expansion to a single response, so pick the
        # first populated capture_results dict across the batch
        # (common case is a single prompt anyway).
