...
class DeepseekV4DecoderLayer(nn.Module):
    ...
    def _steer_and_capture_mhc(
        ...
        ``layer_input`` is the *normed* single-stream sublayer input: the mHC
        pre kernels fuse ``attn_norm`` / ``ffn_norm``, so the pre-norm tensor
        is never materialized. This matches the ``mlp_in`` contract in
        :class:`SteeringHookPoint` (the normed branch input) and applies the
        same way to ``pre_attn`` on this architecture.
